          ...
          <ul tabindex="0" class="dropdown-content z-[1] p-2 shadow-2xl bg-base-300 rounded-box w-52">
              @for(item of selectItems; track item.value) {
                  <li (click)="onSelect(item.value)">
                      <input data-choose-theme type="radio" name="theme-dropdown" class="theme-controller btn btn-sm btn-block btn-ghost justify-start" attr.aria-label="{{item.label}}" value="{{item.value}}"/>
                  </li>
              }
          </ul>
      </div>
...
export class ThemeTogglerComponent {
  ...
  @Input({required: true}) public selectItems: readonly SelectThemeType[] | undefined;
